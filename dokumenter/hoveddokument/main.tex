\documentclass[11pt,a4paper,twoside]{report} % DO NOT CHANGE, UNLESS IF CHANGING STYLE

\usepackage{graphicx,listings,amssymb,amsmath,url,paralist,tabularx,rotating,longtable,lscape}
\usepackage{layouts,hyperref,fancyhdr,listings,color,times,lipsum,chemfig,wasysym}
\usepackage[utf8]{inputenc} 
\usepackage[norsk]{babel} 
\usepackage[absolute]{textpos}
\usepackage{hiofreport}
\usepackage{makeidx}
\usepackage{pdfpages}
\usepackage{float}
\usepackage{caption}
\usepackage{subcaption}
\usepackage{sidecap}
\usepackage{savesym} %kan være unødvendig
\usepackage{glossaries}

\input{metaNO}       % Use this to fill in author, title and other metadata used on front page
\input{defs}        % DO NOT CHANGE

\makeindex

\begin{document}
	\raggedbottom %Test for å fikse merkelig whitespace mellom paragrafer.
% FRONT MATTER
    % DO NOT CHANGE
    %-------------------------------------------------    
    \includepdf[pages={1, {}}] {titlepage_NO} % Make your own catchy PDF title page, named titlepage.pdf 
    \maketitle
	\input{preface}
    \input{abstract}
    \addcontentsline{toc}{chapter}{Sammendrag}
    %-------------------------------------------------

    % OPTIONAL
    %-------------------------------------------------
    \input{acknowledgements}
    \addcontentsline{toc}{chapter}{Takk Til}
    %-------------------------------------------------

	%\cleardoublepage % DO NOT CHANGE    
	\setcounter{tocdepth}{1}
    \tableofcontents  % DO NOT CHANGE    
    %\cleardoublepage % DO NOT CHANGE    
    
    % OPTIONAL
    %-------------------------------------------------
    \listoffigures
    \addcontentsline{toc}{chapter}{Figurliste}
    %\cleardoublepage
    \listoftables
    \addcontentsline{toc}{chapter}{Tabelliste}
    %\cleardoublepage
    \lstlistoflistings
    \addcontentsline{toc}{chapter}{Kodeliste}
    \clearpage
    \addcontentsline{toc}{chapter}{Ordliste}
    \input{glossary}
    \printglossaries
    \cleardoublepage
    %-------------------------------------------------
       
    \pagenumbering{arabic} % DO NOT CHANGE, OR REMOVE IF CHANGING STYLE
    \setcounter{page}{1}       % DO NOT CHANGE, OR REMOVE IF CHANGING STYLE 		
	
% BODY
    % YOU MAY CHANGE TITLES AND ORDER (WITHIN LIMITS)
    %-------------------------------------------------
    \input{introduction}
    \input{analysis}
    \input{planning}
    \input{production}
    \input{evaluation}
    \input{discussion}
    \input{conclusion}
    %-------------------------------------------------


% BACK MATTER
    % DO NOT CHANGE
    %-------------------------------------------------   
    \bibliographystyle{ieeetr}   
    \bibliography{project_bibliography}
    \addcontentsline{toc}{chapter}{Bibliografi}
    %-------------------------------------------------
        
    %\cleardoublepage %DO NOT CHANGE
    %\blankpage          %DO NOT CHANGE
    %\blankpage          %DO NOT CHANGE
\end{document}