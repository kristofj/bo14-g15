...
Ulemper
\begin{itemize}
\item Hvis man ikke bruker alternative måter å kjøre CGI på, kan det fort bli treg utførelsetid på serveren. 
\item All kode kjøres også på serveren, noe som fører til større belastning.
\end{itemize}
...
\item Veldig utbredt språk, så det er mye god dokumentasjon. Kode kjøres av nettleser, noe som fører til lavere belastning på server.
\end{itemize}
Ulemper 
\begin{itemize}
\item Krever litt mer å debugge, men Firebug (utvidelse til Firefox og Chrome) ordner dette relativt greit.
\end{itemize}

\subsubsection{PHP}
Fordeler
\begin{itemize}
\item Veldig lett syntaks og debugging. 
\item Tilgivende språk (det meste fortsetter å kjøre selv om man har kodefeil).
\end{itemize}
Ulemper 
\begin{itemize}
\item Hatt mange problemer med sikkerhet i sin levetid. 
\item All kode kjøres også på serveren, noe som fører til større belastning.\\
\end{itemize}

På bakgrunn av dette har vi bestemt oss for å presentere data ved å hovedsaklig bruke JavaScript med AJAX-teknologi og 'Google Visualization' for tegning av grafer. Som backend for å hente værdata fra databasen og konvertere til JSON trenger vi et språk der kildekode blir kompilert på serveren, og ikke er synlig for bruker. Til dette har vi valgt PHP ettersom det er et utbredt og relativt raskt språk til å være på serversiden.
